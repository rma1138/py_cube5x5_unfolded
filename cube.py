...
def init_cube():
    ...
    for piece in cube_corners:
        ...
        for color_side in [ [side_1, side_2, side_3, piece[0]]
                           ,[side_2, side_1, side_3, piece[1]]
                           ,[side_3, side_2, side_1, piece[2]]]:
            orientation = corner_orientation(color_side[0], color_side[1], color_side[2])
...
def rotate_side(col_row: list[int], rotation: int =0) -> list[int]:
    ...
    raise Exception(f"rotate_side({col_row}, {rotation}): case not handled! Check and fix.")
                        ...
                        for c in range(5):
                            for r in range(5):
                                if cube[side_index][col_index][row_index][1] == cube[s][c][r][1]:
                                    ...
                                    cursor_obj[cursor_obj_index].setWidth(
                                        spacer)
...
def navigate_pos(position: list[int], direction: str, side_selected: bool=False) -> list[int]:
...
def navigate_unfolded(position: list[int], direction: str, side_selected: bool) -> list[int]:
...
def rotate(side: int, rotation: int):
    ...
    for col in range(5):
        for row in range(5):
            ...
            cube[side][rotated_col_row[0]][rotated_col_row[1]]    = saved_side[col][row].copy()
            r = cube[side][rotated_col_row[0]][rotated_col_row[1]]
            cube[side][rotated_col_row[0]][rotated_col_row[1]] = [ r[0], r[1], 1, r[3], r[4] ]
...
def turn(position, rotation):
    ...
    rotate_from_pos_dict = {
        90: rotate_from_pos_90_dict, 270: rotate_from_pos_270_dict}
    this_side = position[0]
    if debug or False:
...
def solve_cube(cursor_pos: list[int], first_color: str='b'):
    ...
    # -----------------------------------------------------------------------------------------------------------
    def is_side_adjacient(first_side: int, second_side: int, third_side: int | None =None) -> bool:
    ...
    def find_piece(piece: str, color: str | None = None) -> list[int]:
        ...
        raise Exception(f"find_piece({piece}, {color}): case not handled!. Check and fix")
    ...
    def relative_direction(from_side: int, to_side: int) -> str:
        ...
        if to_side == 0:
            ...
            if debug or False:
                print("    from side", from_side, "to side",to_side, "direction", to_from_side_dict[to_side][from_side])
    ...
    def is_piece_on_bottom_row(piece: str, from_pos: list[int], to_side: int, side_color: str) -> bool:
        """ find if a piece is on the bottom row in respect to teh target side and color
    ...
    def is_piece_aligned(from_pos: list[int], to_pos: list[int]) -> bool:
        """ find if a piece is aligned below its target position
    ...
    def fill_piece_travels(color: str, pieces: list[str], side: int) -> list[str, list[int], list[int]]:
        # find misplaced piece from / to positions an keep them in piece_travels list
        piece_travels = []

        if pieces[0] in cube_corners:
            # cube_corners
            col_row_dict = {"NW": [0, 0], "EN": [
                4, 0], "SW": [0, 4], "ES": [4, 4]}
            for corner in pieces:
                if color in corner:
                    ...
                    if from_pos != to_pos:
                        ...
                        if debug or False:
                            print("    travel corner", corner,
                                  "from", from_pos, "to", to_pos)
                            break

        if pieces[0] in cube_borders:
            # cube_borders
            border_col_row_dict = { "N": [1, 0], 
                                    "E": [4, 3], 
                                    "S": [3, 4], 
                                    "W": [0, 1]}
    ...
    def solve_first_center(first_side: int, first_color: str):
        ...
        if pos != None and len(pos) == 3: 
            ...
            raise Exception(f"solve_first_center({first_side}, {first_color}): Casen not hanlded. Check and fix.")
        ...
        while len(misplaced_piece_travels) > 0:
            for travel in misplaced_piece_travels:
                ...
                if from_side == to_side:
                    ...
                    elif from_col > to_col and from_row == to_row == 4:
                        turn(from_pos, 90)

                    elif from_col == to_col == 4 and from_row < to_row:
                        turn(from_pos, 90)

                    elif from_col == to_col == 0 and from_row > to_row:
                        turn(from_pos, 90)

                    elif from_col > to_col and from_row == to_row == 0:
                        turn(from_pos, 270)
        ...
        while len(misplaced_piece_travels) > 0:
            for travel in misplaced_piece_travels:
                ...
                if is_piece_reversed(piece, to_side, first_color):
                    if debug or False:
                        print("    case 2: border", piece,
                              "is reversed. Has to be moved down")
                    direction = relative_direction(from_side, to_side)
                    if direction == "Up":
                        from_row = 4
                    elif direction == "Down":
                    ...
                    to_aligned_pos = [
                        to_side, translated_col_row[0], translated_col_row[1]]
                ...
                if from_side == opposite_side[to_side]:
                    if debug or False:
                        print("    case 4: border", piece,
                              "is on the opposite side. Has to be moved to the botton row")

                    move_opposite_corner_to_bottom_row(
                        piece, from_pos, to_side)
                    break   # skip and re-evaluate remaining misplaced pieace
